...
it. Your function should convert it into decimal form. The radix is less
than 37 and greater than 1. The task uses digits and the letters A-Z for
the strings.
...
def checkio(str_number, radix):
    values = dict(map(reversed,
                      enumerate(string.digits + string.ascii_uppercase)))
    total = 0
    for (column, char) in enumerate(reversed(str_number)):
        if values[char] >= radix:
            return -1
        total = total + values[char] * (radix ** column)
    return total
